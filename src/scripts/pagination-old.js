...
export class PaginationController {
  ...
  static initializePanelPagination(section, sectionIndex, panelCount) {
    ...
    for (let i = 0; i < panelCount; i++) {
      const indicator = document.createElement('div');
      indicator.className =
        'pagination-dot panel-inactive hover-active click-feedback';
      ...
      indicator.addEventListener('click', () => {
        ...
        // Show loading state immediately
        this.showLoadingState(sectionIndex, i, 'panel');
  ...
  static updatePanelPagination() {
    ...
    if (
      ...
      if (currentSectionId === 'map') {
        const mapPaginationContainer =
          currentSectionEl.querySelector('.pagination-panel');
        // eslint-disable-next-line no-console
        console.debug(`📍 MAP SECTION UPDATE:`, {
          mapPaginationContainer: !!mapPaginationContainer,
          containerChildren: mapPaginationContainer?.children.length || 0,
          containerDisplay: mapPaginationContainer?.style.display,
          containerVisible: mapPaginationContainer?.offsetHeight > 0,
        });
      }
    }

    // Only hide/show containers if section has changed
    if (sectionChanged) {
      // Hide all panel pagination containers first with animation
      document.querySelectorAll('.pagination-panel').forEach((container) => {
    ...
    document
      ...
      .forEach((dot) => {
        ...
        if (dotSection === currentSection && dotPanel === currentPanel) {
          ...
          // Show the pagination container for this section with animation (only if section changed)
          ...
          if (
            ...
            // Check if this container has been animated before
            const containerId = `section-${dotSection}`;
            if (!this.animatedContainers.has(containerId)) {
              // First time showing this container - animate in
              PaginationAnimationController.initializeDotsForAnimation(
                container
              );
              PaginationAnimationController.animateContainerIn(container, 0.1);
              this.animatedContainers.add(containerId);
            } else {
              // Container has been shown before - just make it visible without animation
            ...
            // Same section, just make container visible without animation
            container.style.display = 'flex';
            PaginationAnimationController.resetDotsToVisible(container);
          } else if (container && container.style.display !== 'none') {
            // Container is already visible, just ensure dots are visible
            PaginationAnimationController.resetDotsToVisible(container);
          }
          ...
          // Hide entire containers for non-current sections (don't hide individual dots)
          const container = dot.closest('.pagination-panel');
          if (container && container.style.display !== 'none') {
            // Animate container out if currently visible
            PaginationAnimationController.animateContainerOut(container, 0);
          }
        }
      });

    // Ensure current section's pagination container is visible even if no dots exist yet
    const sections = appState.getSections();
    if (sections && sections[currentSection]) {
      ...
      if (paginationContainer && paginationContainer.children.length > 0) {
        // Use animated transition instead of direct style change
        if (
          paginationContainer.style.display === 'none' ||
          !paginationContainer.style.display
        ) {
          // Check if this container has been animated before
          const containerId = `section-${currentSection}`;
          if (!this.animatedContainers.has(containerId)) {
            // First time showing this container - animate in
            PaginationAnimationController.initializeDotsForAnimation(
              paginationContainer
          ...
          // Already visible, ensure dots are in correct state
          PaginationAnimationController.resetDotsToVisible(paginationContainer);
        }
        ...
        if (
          ...
          const sectionId = currentSectionEl.id || `section-${currentSection}`;
          if (sectionId === 'map') {
            // eslint-disable-next-line no-console
            console.debug(`✅ MAP PAGINATION NOW VISIBLE:`, {
              sectionId,
              containerDisplay: paginationContainer.style.display,
              containerChildren: paginationContainer.children.length,
              containerOffsetHeight: paginationContainer.offsetHeight,
              containerOffsetWidth: paginationContainer.offsetWidth,
            });
          }
        }
      }
